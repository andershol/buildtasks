...
if (!$infile || !$outfile) { print "Usage : $0 -i <input txt file> -o <output csv file>\n"; exit; }
...
sub structureData {
    ...
    foreach my $line (@lines) {
        ...
        my $build = $name=~/ build$/;
        my $hw =
            ($name=~/^Android (?:Debug |XUL |).*(Armv6|Tegra 250|)/ ? ($build ? "Rev3|Fedora 12" : do{(my $x=$1)=~s/ //;$x}."|Android") :
...
sub writeCsv {
    ...
    foreach my $d (@{$data}) {
        print FILE join("\t", ($d->{'name'}, $d->{'product'}, $d->{'productarch'}, $d->{'hw'}, $d->{'os'}, $d->{'flag'}, $d->{'task'}, $d->{'time'}))."\n";
    }
    close FILE;
}
