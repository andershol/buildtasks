...
if (!$infile || !$outfile) { print "Usage : $0 -i <input csv file> -o <output html file> [-c <cost per hour>]"; exit; }
...
sub readCsv {
	...
	foreach my $line (@lines) {
		my ($name, $product, $productarch, $hw, $os, $flag, $task, $time) = split(/\t/, $line=~s/\s*$//r);
		push($data, {'name'=>$name,'hw'=>$hw, 'product'=>$product, 'productarch'=>$productarch, 'os'=>$os, 'flag'=>$flag, 'task'=>$task, 'time'=>$time=~s/,/./r, });
	}
	$data;
}
...
foreach my $d (@{$data}) {
	...
	if ($d->{'isbuild'}) {
		...
		if (exists $prodPlatform->{$prod} && ($prodPlatform->{$prod}->{'hw'} ne $d->{'hw'} || $prodPlatform->{$prod}->{'os'} ne $d->{'os'})) {
            ...
            $prodPlatform->{$prod}->{'hw'} .= "<br>".$d->{'hw'};
            $prodPlatform->{$prod}->{'os'} .= "<br>".$d->{'os'};
		} else {
    		$prodPlatform->{$prod} = {'hw'=>$d->{'hw'}, 'os'=>$d->{'os'}};
        }
	} else {
...
foreach my $type ('desktop', 'mobile', 'b2g') {
	...
	for (my $i = 0; $i < scalar(@{$colkeys}); $i++) {
		...
		foreach my $col (sort keys %{$cols}) {
			...
			print FILE ($prevt ne $t ? "<th".(exists $colcount->{$t} ? " colspan=".$colcount->{$t} : "")." class='vert".
                ...
                "<div>".($cols->{$col}->{$colkeys->[$i]}=~s/ (\(.*)/<br><small>$1<\/small>/r).($i < 2 ? "<br><small>".formattime($coltime->{$t},1)."</small>" : "")."</div>" : "");
			$prevt = $t;
		}
	}
	...
	my ($prevflag) = ("_");
	foreach my $row (sort { ($a =~s/talos/z\0/r) cmp ($b =~s/talos/z\0/r) } keys %{$rows}) {
